...
@observer
class ConvertPackageForm extends React.Component {
  state = {
    errors: {},
    packagePlans: []
  }
  ...
  async componentDidUpdate(prevProps) {
    if (!prevProps.show && this.props.show) {
      const nestedPackagePlans = await loadPackagePlans(this.props.taskId)

      // Flatten it up
      // TODO: Not sure why we have this logic here....
      const packagePlans = []
        ...
        <div className="flex flex-column flex-auto">
          ...
          <div className="mh4 mt3 flex flex-column gray mb4 ba b--black-20 br2">
            {packagePlans.map(x => (
              <PackageTracking
                key={x.id}
                source_package_id={x.source_package_id}
...
export default ConvertPackageForm
  ...
  onAddPackageID = event => {
    if (event.key === 'Enter') {
      ...
      const data = {
        ...
        source_package_id: this.props.source_package_id,
        tag
      }
...
const loadPackagePlans = async taskId => {
  const url = `/api/v1/sales_products/conversion_plans_by_task/${taskId}`
  const product_plans = await (await fetch(url, httpGetOptions)).json()

  console.log(product_plans)

  const packagePlans = product_plans.reduce((sum, item) => {
