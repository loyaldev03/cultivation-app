...
class ReceiveCannabisForm extends React.Component {
  ...
  onManifestValidate = async () => {
    this.setState(
      ...
      async () => {
        ...
        await DailyTaskStore.loadLocation(
          this.props.facilityId,
          data.manifest.phase
        )
        const plants = data.manifest.plants
        plants.map(e => {
          const location = DailyTaskStore.locations.find(
            f => f.value === e.location_id
          )
          e.location = location
          return e
        })
  ...
  addNewLocation = () => {
    const last_location =
      this.state.plants.length > 0
        ? this.state.plants[this.state.plants.length - 1].location
        : '' //take last location selected
    const newLocation = {
      old_tag: '',
      new_tag: '',
    ...
    const newPlants = [...this.state.plants, newLocation]
  ...
  render() {
    ...
    const growthStages = this.props.growth_stages.map(e => ({
      value: e,
      label: e
    }))
              ...
              <React.Fragment>
                ...
                <div className="w-40 mt3 ttc">
                  ...
                  <DatePicker
                    ...
                    onChange={value =>
                      this.handleChangeDate('plant_date', value)
                    }
                  />
                </div>
                {plants.length > 0 && (
                          ...
                          <div className="flex justify-between mt3">
                            ...
                            <div className="w-20">
                              <Select
                                ...
                                value={e.location}
                                onChange={f =>
                                  this.onChangePlantAttr(e, 'location', f)
                                }
                              />
                            </div>
                            <div className="w-10">
