...
const TaskRow = observer((props) => {
  ...
  const timeSpentToday = moment().startOf('day').seconds(parseInt(dailyTask.attributes.duration)).format('H [hr] m [mn] s [s]')
  const formatIntoDate = (date) => (
    moment(date, 'YYYY-MM-DD').format('M/D/YYYY')
  )
  ...
  return (
    <React.Fragment>
      ...
      <div className={classes}>{formatIntoDate(task.attributes.expected_start_date)}</div>
      <div className={classes}>{formatIntoDate(task.attributes.expected_end_date)}</div>
