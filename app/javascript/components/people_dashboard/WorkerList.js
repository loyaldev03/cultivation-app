...
class OntimeArrivalsWidget extends React.Component {
  ...
  render() {
    return (
      <React.Fragment>
        <h1 className="f5 fw6 black">{PeopleDashboardStore.data_worker_lists.title} - {PeopleDashboardStore.current_workers_length} Workers</h1>
        ...
        {PeopleDashboardStore.data_worker_lists.users.map((e,i) => (
          <div className="flex justify-between mb3">
            ...
            {e.photo_url ? (
              ...
              <span className="f6 fw6 dark-grey ml2">{e.first_name} {e.last_name}</span>
            </div>
            <div className="flex items-center w-50">
              <ProgressBar
                key={i}
              ...
              />
              <span className="f6 fw6 dark-grey ml2">{e.actual} / {e.capacity} Hours</span>
            </div>
            <div className="w-20">
              {e.skills.map(skill =>
                <button className="button mr3 bg-gray bn pa2 tc f6 white">{skill}</button>
              )}
            </div>
            <div className="w-10" href="#" style={{ cursor: 'pointer'}}>
              <button className="button mr3 bg-orange bn pa2 tc f5 white" style={{ cursor: 'pointer'}}>Assign Task</button>
            </div>
          </div>
        ))}
      </React.Fragment>
