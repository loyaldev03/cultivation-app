...
RSpec.describe Cultivation::ValidatePurchaseClone, type: :command do
  ...
  context ".call" do
    it "return error if no purchase clone selected" do
      ...
      expect(result.errors['strain'][0]).to eq('Purchase Clone is not selected')
    end

    it "return nothing if purchase clone selected" do
    ...
    it "return error if purchase clone is insufficient" do
      task = batch1.tasks.first
      task.material_use.new({
        quantity: 11,
        uom: 'kg',
      ...
      expect(result.success?).to be false
      expect(result.errors['strain'].count).to be > 0
      expect(result.errors['strain'][0]).to start_with('Insufficient Purchase Clone')
    end
