...
RSpec.describe DailySystemNotificationWorker, type: :job do
  ...
  context "within 5 days before scheduled batch starts" do
    ...
    it "5 days before batch start date" do
    ...
    it "4 days before batch start date" do
      Time.use_zone(facility.timezone) do
    ...
    it "1 days before batch start date" do
      Time.use_zone(facility.timezone) do
    ...
    it "same day as batch start date" do
      Time.use_zone(facility.timezone) do
  ...
  context "within a day before batch move to next phase" do
    let!(:active_batch) do
      Time.zone = facility.timezone
      start_date = Time.zone.local(2018, 6, 1, 8, 30, 0)
      create(:batch, :active,
    ...
    it "A day before harvest" do
      Time.use_zone(facility.timezone) do
        ...
        Timecop.freeze(current_time) do
          ...
          result = Notification.last
          expect(Notification.count).to eq 1
          expect(result.notifiable_name).to end_with("is scheduled to move into 'harvest' phase tomorrow")
        end
      end
    end
